...
if( $_GET ) {
    ...
    if( $res && $res -> num_rows > 0 ) {
        $edit_info = $res -> fetch_all(MYSQLI_ASSOC);
    }
//    $edit_info = $edit_info[0];
//    print_r( $edit_info );exit;
    $movie_type_str = prepare_movie_type( $edit_info['movie_type'] );
...
//    print_r( $movie_type );exit;
    ...
    $sql="UPDATE movie SET {$sets} {$where}";
//    print_r( $id );exit;
    $res=$conn -> query($sql);
    if($res){
...
function prepare_movie_type( $movie_type ) {
    ...
    foreach( $movie_type_config as $k => $v ) {
        if( strpos( $movie_type, $v ) !== FALSE ) {
            $res .= "<input type='checkbox' checked='checked' name='movie_type[]' value='{$v}''>" . $v;
        }else{
            $res .= "<input type='checkbox' name='movie_type[]' value='{$v}''>" . $v;
        }
