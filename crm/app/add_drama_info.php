...
//    $short_title = trim(filter_input(INPUT_POST,'short_title',FILTER_SANITIZE_STRING));
    ...
    $drama_id = trim(filter_input(INPUT_POST,'drama_id',FILTER_VALIDATE_INT));
...
//        "main_pic" => $main_pic,
    ...
    $sql="INSERT INTO drama_info ({$keys}) VALUES ({$values})";
//    print_r( $sql );exit;
    $res = $conn -> query($sql);
    if($res){
        header("refresh:1;url=drama_info.php");
        echo "操作成功";
    }else{
        header("refresh:1;url=drama_info.php");
        echo "操作失败";
...
include("../template/add_drama_info.php");