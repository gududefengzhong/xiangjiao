...
if( $_GET ) {
    ...
    $sql = "SELECT * FROM drama WHERE id={$id}";
    $res = $conn -> query( $sql );
...
//    print_r( $_POST );exit;
    ...
    $drama_type = implode(",",$_POST['drama_type']);
...
//    print_r( $sql );exit;
    $res=$conn -> query($sql);
    if($res){
        header("refresh:2;url=drama.php");
        echo "编辑成功";
    }else{
        header("refresh:2;url=drama.php");
        echo "编辑失败";
...
function prepare_drama_type( $drama_type ) {
    ...
    foreach( $drama_type_config as $k => $v ) {
        if( strpos( $drama_type, $v ) !== FALSE ) {
            $res .= "<input type='checkbox' checked='checked' name='drama_type[]' value='{$v}''>" . $v;
        }else{
            $res .= "<input type='checkbox' name='drama_type[]' value='{$v}''>" . $v;
        }
...
include("../template/edit_drama.php");